...
- 前端“策略实验”页签会读取 `public/reports/backtests/latest.json` 和 `public/reports/backtests/history/index.json`，展示主策略当日选股、审美观察池、历史基准胜率、因子归因、分桶回测、最近每日流水和策略归档。后续新增策略数据时优先保证网页可见。
